...
final class CommandCenterPairing: ObservableObject {
    ...
    func discoverAndPairIfPossible() -> Bool {
        ...
        do {
            try pair(pluginURL: suggestion.pluginURL, projectVaultURL: suggestion.projectVaultURL, personalProjectVaultURL: suggestion.personalProjectVaultURL, captainsLogURL: suggestion.captainsLogURL, researchJournalURL: suggestion.researchJournalURL, plannerFeedURL: suggestion.plannerFeedURL, continuityFeedURL: suggestion.continuityFeedURL, meetingNotesURL: suggestion.meetingNotesURL, weatherAddress: weatherAddress)
            return true
        } catch { return false }
    }
    ...
    var suggestedPairing: SuggestedPairing? {
        ...
        return SuggestedPairing(pluginURL: pluginURL, projectVaultURL: projectVaultURL, personalProjectVaultURL: Self.discoveredPersonalProjectVaults().first, captainsLogURL: Self.discoveredCaptainsLogs().first, researchJournalURL: Self.discoveredResearchJournals().first, plannerFeedURL: Self.discoveredPlannerFeeds().first, continuityFeedURL: Self.discoveredContinuityFeeds().first, meetingNotesURL: Self.discoveredMeetingNotes().first)
    }

    func pair(pluginURL: URL, projectVaultURL: URL, personalProjectVaultURL: URL?, captainsLogURL: URL?, researchJournalURL: URL?, plannerFeedURL: URL?, continuityFeedURL: URL?, meetingNotesURL: URL?, weatherAddress: String) throws {
    ...
    func clear() {
        pluginURL = nil; projectVaultURL = nil; personalProjectVaultURL = nil; captainsLogURL = nil; researchJournalURL = nil; plannerFeedURL = nil; continuityFeedURL = nil; meetingNotesURL = nil; weatherAddress = ""
        [Key.plugin, Key.projectVault, Key.personalProjectVault, Key.captainsLog, Key.researchJournal, Key.plannerFeed, Key.continuityFeed, Key.meetingNotes, Key.weatherAddress].forEach(UserDefaults.standard.removeObject(forKey:))
        revision += 1
    }
    ...
    private static func discoveredPersonalProjectVaults() -> [URL] {
        ...
        return FileManager.default.fileExists(atPath: url.appendingPathComponent("03 Projects", isDirectory: true).path) ? [url] : []
    }
...
enum PairingError: LocalizedError {
    case invalidPlugin, invalidProjectVault, invalidPersonalProjectVault, invalidCaptainsLog, invalidResearchJournal, invalidPlannerFeed, invalidContinuityFeed, invalidMeetingNotes
    var errorDescription: String? {
        ...
        case .invalidPersonalProjectVault: "Select the Personal Project Knowledge vault that contains a 03 Projects folder."
...
struct CommandCenterPairingView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 20) {
            ...
            HStack {
                if pairing.isPaired { Button("Forget this Mac") { pairing.clear(); pluginURL = nil; projectVaultURL = nil; personalProjectVaultURL = nil; captainsLogURL = nil; researchJournalURL = nil; plannerFeedURL = nil; continuityFeedURL = nil; meetingNotesURL = nil; weatherAddress = "" }.foregroundStyle(.red) }
    ...
    private func save() {
        ...
        do {
            try pairing.pair(pluginURL: pluginURL, projectVaultURL: projectVaultURL, personalProjectVaultURL: personalProjectVaultURL, captainsLogURL: captainsLogURL, researchJournalURL: researchJournalURL, plannerFeedURL: plannerFeedURL, continuityFeedURL: continuityFeedURL, meetingNotesURL: meetingNotesURL, weatherAddress: weatherAddress)
            dismiss()
